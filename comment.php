<?php
if ('comment.php' == basename($_SERVER['SCRIPT_FILENAME']))
...
<?php if ($comments) : ?>
    ...
    <div class="small">
        ...
        <?php if ('open' == $post->ping_status) { ?><span class="trackbacklink"><a href="<?php trackback_url() ?>"
                                                                                   title="Copy this URI to trackback this entry.">Trackback Address</a>
            </span><?php } ?>
    </div>
    ...
    <ol class="commentlist">
        ...
        <?php foreach ($comments as $comment) : ?>
            ...
            <li <?php echo $oddcomment; ?>id="comment-<?php comment_ID() ?>">
                ...
                <p class="small"><?php comment_date('F jS, Y') ?> at <?php comment_time() ?> (<a
                            href="#comment-<?php comment_ID() ?>"
                            title="">#</a>) <?php edit_comment_link('edit', '&nbsp;&nbsp;', ''); ?></p>
...
<?php if ('open' == $post->comment_status) : ?>
    ...
    <?php else : ?>
        ...
        <form action="<?php echo get_option('siteurl'); ?>" method="post" id="commentform">
            ...
            <?php else : ?>
                ...
                <p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22"
                          tabindex="2"/>
                    <label for="email"><small>Email (will not be
                            published) <?php if ($req) echo "(required)"; ?></small></label></p>
